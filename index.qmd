...
## Back to the Future: future.p2p - it's easy!
...
## End-user can choose from many built-in parallel backends
...
These are all based on the **parallel** package.
...
The **future.mirai** package wraps the **mirai** package that is an
alternative to the built-in **parallel**-based `multisession` backend:
...
## future.p2p - A peer-to-peer parallel cluster

![](images/future-logo.png){.absolute top=125 left=1200 width="250"}

* Share compute resources among friends
...
library(future)
plan(future.p2p::cluster, cluster = "alice/p2p")
...
![**~460 CRAN packages** depend directly on the **future** package - it grows 3× faster than **foreach** at 1200 reverse dependencies](images/revdep_over_time_on_CRAN.png)

:::

::: {.column width="49%" .fragment fragment-index=2}

![The **future** packages is among **the 0.7% most downloaded** CRAN packages](images/downloads_over_time_on_CRAN.png)
...
* Install `wormhole` (for P2P file sharing)
