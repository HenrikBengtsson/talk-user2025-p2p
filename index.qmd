...
library(future.p2p)
plan(pico_p2p, cluster = "alice/p2p")
...
```r
{alice}$ R --quiet
> future.p2p::pico_p2p_cluster(cluster = "alice/p2p", users = c("bob", "carol"))
```
...
```r
{alice}$ Rscript -e future.p2p::pico_p2p_cluster --cluster=alice/p2p --users=bob,carol
```
